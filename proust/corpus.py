...
def preprocess(text, use_aliases=True, aliases=None):
    ...
    if use_aliases:
        ...
        text = apply_alias_replacements(text, replacements)
    return text


def apply_alias_replacements(text, replacements):
    if not replacements:
        return text

    placeholders = {}
    for index, alias in enumerate(sorted(replacements, key=len, reverse=True), start=1):
        placeholder = f"__ALIAS_{index}__"
        placeholders[placeholder] = replacements[alias]
        pattern = rf"(?<!\w){re.escape(alias)}(?!\w)"
        text = re.sub(pattern, placeholder, text)

    for placeholder, canonical_name in placeholders.items():
        text = text.replace(placeholder, canonical_name)
...
def get_proust_page(id, source="file"):
    if source == "file":
        ...
        print(f"Loading page from file: {file_path.relative_to(REPO_ROOT)}")
...
def write_proust_pages(start=1, end=486, source="web"):
    for id in range(start, end + 1):
        ...
        file_path = DATA_DIR / f"islt_fr_{id:03}.html"
        print(f"Writing file {file_path.relative_to(REPO_ROOT)}")
